...
echo -e "${cya}This machine is gonna build - ${bldcya}ThinkingBridge Stable1 ${txtrst}";
...
# Sync the latest ThinkingBridge Sources
echo -e ""
if [ "$SYNC" == "sync" ]
then
   if [ "$(which repo)" == "" ]
   ...
   echo -e "${bldblu}Syncing latest ThinkingBridge sources ${txtrst}"
   repo sync -j"$THREADS"
   echo -e ""
fi
...
then
   ...
   lunch "thinkingbridge_crespo-userdebug"
   make -j"$THREADS" otapackage
   echo -e "${bldblu}grouper ${txtrst}"
   lunch "thinkingbridge_grouper-userdebug"
   make -j"$THREADS" otapackage
   echo -e "${bldblu}i9100g ${txtrst}"
   lunch "thinkingbridge_i9100g-userdebug"
   make -j"$THREADS" otapackage
   echo -e "${bldblu}maguro ${txtrst}"
   lunch "thinkingbridge_maguro-userdebug"
   make -j"$THREADS" otapackage
   echo -e "${bldblu}mako ${txtrst}"
   lunch "thinkingbridge_mako-userdebug"
   make -j"$THREADS" otapackage
else
   # Lunch Device
   echo -e ""
   ...
   brunch "thinkingbridge_$DEVICE-userdebug" -j"$THREADS";
   echo -e ""
fi
